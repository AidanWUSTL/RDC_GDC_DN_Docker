...
# Start making a .sh file to submit to queue
	QUEUEFILE=${WD}/${Subject}/${Subject}_HCP_PreFreesurfer_no_acpc_sbatch_swap.sh
	echo "#!/bin/bash" > ${QUEUEFILE}
	echo "#SBATCH --job-name=${Subject}_HCP_PreFreesurfer_sbatch_no_acpc_swap" >> ${QUEUEFILE}
	echo "#SBATCH --time=3:0:0"  >> ${QUEUEFILE}
	echo "#SBATCH --nodelist=node[18]" >> ${QUEUEFILE}
	echo "#SBATCH --partition=free" >> ${QUEUEFILE}
	...
	T1_loc=`find ./ -name "*T2w*json" -print | cut -d "." -f2`
	T2_loc=`find ./ -name "*T1w*json" -print | cut -d "." -f2`
	SEFM_PA=`find ./ -name "*SpinEchoFieldMap*PA*json" -print | cut -d "." -f2 | head -1`			#  I am adding the "head" portion so I only grab the first one, I only downloaded the first set for this
	...
	SEFM_AP=`find ./ -name "*SpinEchoFieldMap*AP*json" -print | cut -d "." -f2 | head -1`
	...
	#echo "module load workbench/1.5.0 fsl/6.0.4"				>> ${QUEUEFILE}
	#echo "conda deactivate"						>> ${QUEUEFILE}
	#echo "source /export/anaconda/anaconda3/anaconda3-2020.07/bin/activate" 		>> ${QUEUEFILE}
	#echo "source activate python_383" 					>> ${QUEUEFILE}		# we want to be ABSOLUTELY sure my python comes first  : )  Thanks Sri!
	#echo ""									>> ${QUEUEFILE}
	...
	#dennis commented out this line 060523
	...
	echo ""
	echo "Submitting job to cluster for PreFreeSurferPipeline.sh without ACPC alignment and T1/T2 swap subject " ${Subject}
	echo ""
	#chmod 777 ${QUEUEFILE}
